...
/*  consts are used only at server side
    clients query server to get these setting values
*/
static const int ONE_CYCLE_BLOCKS = 5;  // how many block to send in one cycle

static const int PACKET_SIZE = 450; //UDP datagram 512 bytes for internet transfer, data payload

static const int FRAGMENT_SIZE = PACKET_SIZE; // let fragments size equals to packet size

static const int SEND_FRAGMENT_INTERVAL = 10; //msec
